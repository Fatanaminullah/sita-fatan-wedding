...
export function Events({ invited, pax, candid }: { invited: EventKey[]; pax: number; candid: boolean }) {
  ...
  return (
    <section ref={ref} id="events" className="inv-events" aria-label="Events">
      {/* The opener belongs to the hijab invitation only (owner,
          2026-09-22). There it is the date and the places kept in this
          guest's name, which read as one line of address. The non-hijab
          version says neither, so it opens straight onto the first door
          rather than on a date with nothing under it. */}
      {candid ? null : (
