...
- **Ushers have no policy on `guests`, and read the door roster through a function instead.** *Amended 2026-08-29; this rule previously read "Ushers have no guest-list read. An usher must never be able to enumerate guests."* Two operational facts overturned it. A QR that will not scan — cracked screen, flat battery, a guest who never opened the message — must not become a blocked door with a queue behind it, and an usher who cannot tell a VIP from anyone else cannot do the job the VIP tier exists for. So an usher can now list and search everyone invited to one event.

  What the original rule protected is kept. Ushers still hold **no SELECT policy on `guests`**, because row-level security cannot hide a column: any policy wide enough to serve a door would also hand over `phone`. Both reads go through SECURITY DEFINER functions that return the door's columns and no others — `guest_by_rsvp_token(token, event)` for one guest from their ticket, `guest_roster_for_event(event, query)` for the list. `phone`, `rsvp_token` and `public_slug` are unreachable by construction rather than by discipline.

  **`note` is returned by the roster, and searched.** *Amended 2026-08-30.* It was excluded at first on the grounds that it is internal and often about the guest rather than for them. That was wrong about how the column is actually used: it holds the group a guest belongs to ("Keluarga A", "Teman kantor"), which is how the couple tell one Wati from another at a door, and searching it is how an usher pulls up a whole family at once. It stays out of `guest_by_rsvp_token`, whose result renders on a tablet facing the guest.

  Both functions bypass RLS by definition, so their own role guard is the only access control they have, and it must fail closed. Guard them with `coalesce(current_profile_role(), '') not in (...)`: `current_profile_role()` returns NULL for a caller with no profile row, `NULL not in (...)` evaluates to NULL, and plpgsql treats a NULL condition as false, so the bare form lets an unknown caller straight through. That exact bug shipped to staging on 2026-08-29 and was caught by calling the function rather than by reading it. `tests/rls/door-functions.test.ts` pins the behaviour.
