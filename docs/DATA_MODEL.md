...
- **Ushers have no policy on `guests`, and read the door roster through a function instead.** *Amended 2026-08-29; this rule previously read "Ushers have no guest-list read. An usher must never be able to enumerate guests."* Two operational facts overturned it. A QR that will not scan — cracked screen, flat battery, a guest who never opened the message — must not become a blocked door with a queue behind it, and an usher who cannot tell a VIP from anyone else cannot do the job the VIP tier exists for. So an usher can now list and search everyone invited to one event.
  ...
  What the original rule protected is kept. Ushers still hold **no SELECT policy on `guests`**, because row-level security cannot hide a column: any policy wide enough to serve a door would also hand over `phone` and `note`. Both reads go through SECURITY DEFINER functions that return the door's columns and no others — `guest_by_rsvp_token(token, event)` for one guest from their ticket, `guest_roster_for_event(event, query)` for the list. `phone`, `note`, `rsvp_token` and `public_slug` are unreachable by construction rather than by discipline.
