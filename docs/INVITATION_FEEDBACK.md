...
| 21 | scroll terasa terlalu cepat untuk user awam | `smooth-scroll.tsx`: touch now goes through Lenis (`syncTouch`, multiplier 0.8, shorter inertia), wheel 0.75; vow and couple wrappers stretched (450 / 620lvh). Needs an iOS check: if the synthetic touch fights Safari, turn `syncTouch` off and keep the longer sections. |
